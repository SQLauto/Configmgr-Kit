...
where s.PackageID not in (select PackageID from v_DistributionPoint where ServerNALPath like '%derbfPS01%')
 and ServerNALPath like '%selucmdp01%'
...
--and ((p.sourcesize)/1024)<'3000' 
and p.Description not like '%lund%' and p.PackageType not like '5' 
--and len(p.name)>49
order by 8,4
