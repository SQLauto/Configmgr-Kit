...
'
' niall brady windows-noob.com (c) 2015/3/14
' updated 2015/3/26 to add a retry possibility, in case network cable was unplugged etc..., increased network wait time from 12 seconds to 15 for UEFI computers...
' updated 2015/4/16 to check for Storage
' updated 2015/4/22 added new partition check and logging
...
For Each objDisk in colDisks	
	if (objDisk.DriveType = 3) AND (objDisk.DeviceID <>"X:") then
		...
		' if we got here then the drive types..could not be determined., maybe not partitioned ?
		LogText "DeviceID: " & objDisk.DeviceID & " DriveType: could not be determined."	
		CheckPartitions = True
	End if
Next
LogText "CheckForSata = " & CheckForSata
' check if a disk was found as the first physical disk, but could not be determined..
...
If CheckForSata >= 1 then
		LogText "A disk was detected, it may be not partitioned or have an unknown partition type. Partitioning will be handled by the task sequence therefore continuing.."
	else
end if
...
ServersToPing=Array("10.163.60.91")   
...
Set tmpObj = Nothing
Set tmpItem = Nothing
' look for devices that match SATA, STORAGE, SCSI or SAS

Set objWMIService = GetObject("winmgmts:\\" & strComputer & "\root\cimv2")
Set colItems = objWMIService.ExecQuery("Select * from Win32_PNPEntity where (description LIKE '%sata%') OR (description LIKE '%Storage%') OR (description LIKE '%SCSI%') OR (description LIKE '%SAS%')",,48)
